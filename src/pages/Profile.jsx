...
import { 
  ...
  FaShieldAlt, FaKey, FaImage, FaSync, FaMobileAlt, FaQrcode
} from 'react-icons/fa';
...
const Profile = () => {
  ...
  const refreshUserData = useCallback(async () => {
    ...
    try {
      ...
      if (response.data?.data) {
        ...
        setTwoFactorEnabled(freshUser.twoFactorEnabled || false);
  ...
  // 🔥 Get current banner - ALWAYS from user state (source of truth)
  const getCurrentBanner = useCallback(() => {
    if (bannerPreview) return bannerPreview;
    if (user?.bannerImage) return user.bannerImage;
    return defaultBanner;
  }, [bannerPreview, user?.bannerImage, defaultBanner]);

  // Clear previews when actual images change
  useEffect(() => {
    setPreviewUrl(null);
      ...
      <div className="max-w-4xl mx-auto space-y-6 px-4 sm:px-0">
        ...
        >
          ...
          <div className="relative h-32 sm:h-36 md:h-44">
            ...
            <button
              ...
              disabled={refreshing}
              className="absolute top-3 right-3 p-2 bg-black/50 hover:bg-black/70 backdrop-blur-sm text-white rounded-lg transition-all z-10"
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end -mt-12 sm:-mt-14 md:-mt-16 gap-3 sm:gap-4">
              ...
              <div className="relative flex-shrink-0 z-10">
                <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full border-3 sm:border-4 border-white dark:border-gray-800 bg-gradient-to-br from-indigo-100 to-blue-100 dark:from-indigo-900 dark:to-blue-900 flex items-center justify-center overflow-hidden shadow-xl">
                  ...
                  <span
                    className={`text-3xl sm:text-4xl font-bold text-indigo-600 dark:text-indigo-300 ${displayImageUrl ? 'hidden' : 'flex'} items-center justify-center`}
        ...
        >
          ...
          <div className="space-y-2 sm:space-y-3">
            {/* 2FA Card */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 p-3 sm:p-4 bg-gray-50 dark:bg-gray-700/40 rounded-xl border border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center flex-shrink-0">
                  <FaMobileAlt className="text-indigo-600 dark:text-indigo-400 text-sm" />
                </div>
                <div>
              ...
              <button 
                ...
                className={`w-full sm:w-auto px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors ${
                  twoFactorEnabled 
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 p-3 sm:p-4 bg-gray-50 dark:bg-gray-700/40 rounded-xl border border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center flex-shrink-0">
                  <FaKey className="text-indigo-600 dark:text-indigo-400 text-sm" />
                </div>
                <div>
              ...
              <button className="w-full sm:w-auto px-4 py-2 rounded-lg text-xs sm:text-sm font-medium border border-indigo-600 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors">
                Update
              </button>
            </div>
          </div>
